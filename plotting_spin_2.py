...
# PLOTTING
# filepaths = ['../data/spin_2cx.txt', '../data/spin_2zn.txt']
filepaths = ['../data/spin_1cx.txt', '../data/spin_1zn.txt']
# filepaths = ['../data/spin_runs_maxiter2000/spin_2cx.txt', '../data/spin_runs_maxiter2000/spin_2zn.txt']
titles = [r'MBVQE (4 CX)', r'MBVQE ($R_{Z^{\otimes 4}}$)']
fig_title = 'Depth 1 MBVQE'
plt.rcParams.update({
    'text.usetex': True,
    'font.size': 12
})

fig, axs = plt.subplots(len(filepaths), 1, sharex=True, sharey=True, figsize=[6, 6])

histograms = []
for ax, path, title in zip(axs, filepaths, titles):
...
axs[1].set_xlabel('Spin coupling strength (J/B)')
axs[0].set_ylim(-7.9, -2.05)
fig.suptitle(fig_title)
plt.tight_layout()
cbar = fig.colorbar(histograms[-1], ax=axs, cmap=my_colourmap, aspect=35)
